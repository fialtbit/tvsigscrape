...
# initialise logger for writing logs (available in CloudWatch)
...
response = {}
exchange = "BINANCE"
market_list = ['ETHUSDT', 'ADAUSDT', 'BNBUSDT', 'NEOUSDT', 'KEYBTC']
candle_list = ['|15', '|60', '|240', '']  # options "|15" = minutes; "" = day; "|1M" = month; "|W" = week
...
def create_csv_string(response, market):
    """
    ...
    text = "\n".join([",".join(["market", "signal", "value"]), text])
...
def write_to_s3(text, exchange, market, candle, timestamp):
    ...
    logger.info(
        "Scraping {} market for {} candle".format(
            market,
            candle
        )
    )
    ...
    candle = candle.replace('|', '') if candle != '' else '1D'
...
def main(timestamp):
    ...
    for market in market_list:
        for candle in candle_list:
            ...
            logger.info("Response pulled for {} market and {} candle".format(market, candle))

            text = create_csv_string(response, market)
            logger.info("Text created for {} market and {} candle".format(market, candle))

            write_to_s3(text, exchange, market, candle, timestamp)
            logger.info("S3 bucket written for {} market and {} candle".format(market, candle))
